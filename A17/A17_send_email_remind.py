...
class Send_Email:
    def __init__(self):
        ...
        self.mail_receivers = ['915228208@qq.com']      # 设置收件人邮箱地址  2591494509@qq.com
    ...
    def read_excel_add(self,excel_path,excel_name):
        # 构造附件
        atta = MIMEText(open(excel_path,'rb').read(),'base64','utf-8')
        # 设置附件信息
        atta["Content-Disposition"] = 'attachment; filename="{}'.format(excel_name)
        # 添加附件到邮件信息中去
        self.mm.attach(atta)
    ...
    def send_email(self):
        ...
        try:
            ...
            logging.error('%-40s / %-10s','邮件发送失败 / Mail sent failed',datetime.datetime.now())
            print(e)
        # 关闭SMTP对象
        stp.quit()
    def run(self):
        # 设置邮件头部内容
        # 邮件主题
        subject_content = '''非法人脸提醒'''
        # 设置返送着，注意遵守格式。里面邮箱为发件人邮箱
        self.mm['From'] = "人脸助手<{}>".format(self.mail_sender)                     # sender_name 可以是接收者收显示的名字
        # 设置接收者，注意严格遵守格式，里面为接收者邮箱
        self.mm["TO"] = "尊敬的用户<{}>".format(self.mail_receivers)           # "receiver_1_name<******@qq.com>,receiver_2_name<******@outlook.com>"
        # 设置邮箱主题
...
def main():
    em = Send_Email()
    em.run()
    # em.read_image_add("480_640.jpg","480_640.jpg")
    # em.read_excel_add("480_640.jpg","480_640.jpg")
    em.send_email()
